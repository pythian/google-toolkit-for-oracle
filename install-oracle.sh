...
GETOPT_OPTIONAL="$GETOPT_OPTIONAL,iscsi-portals:,gcnv-host-map-json:,storage-backend:,gcnv-fulfillment-json:"
...
while true; do
  case "$1" in
    ...
    --iscsi-portals) YAML_VARS["iscsi_portals"]="$2"; shift 2 ;;
    --gcnv-host-map-json) YAML_VARS["gcnv_host_map_json"]="$2"; shift 2 ;;
    --storage-backend) YAML_VARS["ora_storage_backend"]="$2"; shift 2 ;;
    --gcnv-fulfillment-json) YAML_VARS["gcnv_fulfillment_json"]="$2"; shift 2 ;;
    --tls-secret) 
...
if [ "$HELP_ONLY" = true ]; then
  ...
  echo "  --iscsi-portals <ips>        Comma-separated GCNV iSCSI portal IPs (e.g. '10.0.0.10,10.0.0.11'). Logs in LUNs as /dev/mapper/* before storage setup."
  echo "  --gcnv-host-map-json <json>  Per-host GCNV map keyed by hostname {portals, multipath_aliases}. Normally generated by Terraform when storage_backend=gcnv."
  echo "  --storage-backend <name>     Storage backend for DATA/RECO: 'gce-pd' (default) or 'gcnv'. Normally set by Terraform."
  echo "  --gcnv-fulfillment-json <json> Per-host GCNV fulfillment plan (host group, region, project, block devices) consumed by the gcnv-provision role. Normally generated by Terraform when storage_backend=gcnv."
